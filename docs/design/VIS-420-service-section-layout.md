...
> ⚠️ **이 문서는 구현 완료된 일회성 작업 설계서입니다 (역사 기록용).**
> 현재 서비스 카드 목록·가격·구조는 여기와 다릅니다 — 최신 명세는 `layout-home.md` Section 5 참조.
> - 서비스 구성이 변경됨 (6종 동일하나 AI솔루션→AI도입, 리뉴얼→웹사이트리뉴얼, 챗봇 신규 등)
> - 카드에 아이콘(`icon`) 대신 `Mascot` 컴포넌트 사용으로 교체됨
> - 가격도 변경됨 (예: AI도입 200만→100만, 챗봇 신설 50만, 모의해킹 신설 29만 등)
