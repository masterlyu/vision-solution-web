...
> VIS-5966 루틴 산출물 | AI 디자이너 작성 | 2026-09-01 | 태그 현황 업데이트 2026-09-15 (VIS-6729)
...
| 기본값(fallback) | 온라인마케팅 · SNS 마케팅 · SEO 최적화 등 미분류 태그 | `primary/10` | `primary` |

> **코드 현황(2026-09-15):** `TAG_COLORS` 상수에는 파랑·빨강·primary·초록·앰버 버킷 태그 10개만 명시됨. `웹 보안 진단`, `홈페이지 진단`, `AI 에이전트`, `업무 자동화` 등 신규 태그는 fallback(`primary`)으로 처리 중.
> 빨강 버킷에 `웹 보안 진단` 추가를 권장하나, 1개 포스트에만 사용 중이므로 우선순위 낮음 — 콘텐츠 팀 판단에 따라 `보안 경고`·`보안 점검` 통일 우선 검토.
