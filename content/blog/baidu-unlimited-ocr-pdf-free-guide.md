...
거래처 계약서가 60페이지입니다. 꼭 확인해야 할 위약 조항은 어딘가 끼어 있습니다. 담당자가 프린트해서 밑줄 치고, 핵심 추려 메모하고, 다시 검토 요청하는 과정이 반복됩니다. ChatGPT에 올리면 어떨까요 — 파일이 길면 중간 내용을 흘려버리거나, 페이지 단위로 잘라 보내야 해서 맥락이 끊깁니다.

바이두가 2026년 6월 22일 공개한 **Unlimited-OCR**은 이 문제를 다르게 접근합니다. 문서를 자르지 않고, 처음부터 끝까지 한 번에 읽습니다. 출시 5일 만에 GitHub 별이 1만 개를 넘었습니다.
...
대부분의 OCR 도구는 PDF를 페이지 단위 이미지로 변환한 뒤 각 페이지를 따로 처리합니다. 이 방식의 문제는 페이지 경계를 넘는 맥락 처리입니다. 한 조항이 39페이지에서 시작해 40페이지에서 끝나면, 두 처리 결과를 이어붙여야 하고 그 이음새에서 오류가 생깁니다.

Unlimited-OCR이 쓴 기술은 **R-SWA(Reference Sliding Window Attention)**입니다. 핵심은 KV 캐시를 평탄하게(flat) 유지하는 것입니다. 문서가 아무리 길어도 모델이 유지해야 할 메모리 크기가 늘지 않습니다. 덕분에 40+ 페이지 문서를 처음부터 마지막까지 같은 정확도로 처리합니다.

성능은 수치로 나왔습니다. OmniDocBench v1.6 기준 **93.9%**, DeepSeek OCR 대비 긴 문서에서 **약 35% 빠른 속도**입니다.
...
RTX 3060 (12GB) 이상이면 FP16 풀 모델을 로컬에서 쓸 수 있습니다. 8GB GPU라면 INT4 양자화 버전으로 실행 가능합니다. 그마저도 없다면 아래 HuggingFace 웹 데모로 먼저 테스트해보면 됩니다.

**솔직한 한계:** 한국어 문서 처리 성능은 별도 검증이 필요합니다. 영어·중문 중심으로 훈련된 모델이라 한글 계약서·보고서에서 어느 정도 정확도가 나오는지 실제 테스트로 확인해야 합니다.
...
[HuggingFace Space](https://huggingface.co/spaces/baidu/Unlimited-OCR)를 열면 설치 없이 바로 쓸 수 있습니다. Zero GPU 방식으로 무료 운영되므로 사용량이 몰릴 때는 대기가 생길 수 있습니다.
...
처음에는 1~3페이지 분량으로 먼저 테스트해보고, 결과가 만족스러우면 분량을 늘려가는 게 좋습니다.
...
웹 데모는 편리하지만 기밀 문서를 외부 서버에 올리는 건 조심스럽습니다. 계약서, 내부 보고서, 고객 데이터가 담긴 파일이라면 로컬 설치가 맞습니다.
...
GitHub 저장소([github.com/baidu/Unlimited-OCR](https://github.com/baidu/Unlimited-OCR))에 OpenAI 호환 API 서버 실행 예시도 포함되어 있습니다. 사내 서버에 올려두면 직원들이 웹 브라우저로 접근하는 방식도 가능합니다.
...
바이두가 3B 모델로 이 결과를 낸 게 흥미롭습니다. "긴 문서 처리 = 더 큰 모델"이 공식처럼 통했는데, R-SWA는 그 공식을 구조로 깼습니다. 중소기업 입장에서 실질적인 의미는 하나입니다 — 클라우드 구독 없이, 사무실 서버 한 대로도 돌릴 수 있는 문서 AI가 생겼습니다. 계약서 검토, 제품 카탈로그 텍스트 추출, 거래명세서 자동 입력 같은 반복 업무에 붙여볼 만합니다. MIT 라이선스라 비용 부담 없이 시도할 수 있다는 점이 진입 장벽을 낮춰줍니다. 한국어 성능 검증은 필수지만, 웹 데모에서 5분이면 판단할 수 있습니다.
