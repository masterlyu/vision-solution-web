...
export default function ContactPage() {
  ...
  if (done) return (
    <div className="section min-h-screen pt-24 flex items-center justify-center">
      <div className="card border-brand/30 bg-brand/5 text-center max-w-md w-full p-12">
        ...
        <p className="text-neutral-400 text-sm">영업일 기준 1일 내 이메일로 연락드립니다.</p>
      </div>
    </div>
  )
    ...
    <div className="section min-h-screen pt-24">
      <div className="container-base">
        <div className="text-center mb-12">
          ...
          <p className="text-neutral-400 text-lg">48시간 내 답장드립니다. 견적은 무료입니다.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="space-y-4">
            <div className="card">
              ...
              <a href="mailto:biztalktome@gmail.com" className="text-brand text-sm hover:underline">biztalktome@gmail.com</a>
            </div>
            <div className="card">
              <div className="text-2xl mb-2">🌐</div>
              <h3 className="text-white font-bold mb-1">웹사이트</h3>
              <span className="text-neutral-400 text-sm">visionc.co.kr</span>
            </div>
            <div className="card">
              <div className="text-2xl mb-2">⏰</div>
              <h3 className="text-white font-bold mb-1">응답 시간</h3>
              <span className="text-neutral-400 text-sm">영업일 기준 48시간 내</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <form onSubmit={handleSubmit} className="card space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-neutral-500 text-xs font-medium mb-1.5 block">이름 <span className="text-brand">*</span></label>
                  <input type="text" value={form.name} onChange={set('name')} placeholder="홍길동" required className={inputCls} />
                </div>
                <div>
                  <label className="text-neutral-500 text-xs font-medium mb-1.5 block">이메일 <span className="text-brand">*</span></label>
                  <input type="email" value={form.email} onChange={set('email')} placeholder="example@company.com" required className={inputCls} />
                </div>
              </div>
              <div>
                <label className="text-neutral-500 text-xs font-medium mb-1.5 block">회사명</label>
                <input type="text" value={form.company} onChange={set('company')} placeholder="(선택)" className={inputCls} />
              </div>
              <div>
                <label className="text-neutral-500 text-xs font-medium mb-1.5 block">문의 서비스 <span className="text-brand">*</span></label>
              ...
              <div>
                <label className="text-neutral-500 text-xs font-medium mb-1.5 block">문의 내용 <span className="text-brand">*</span></label>
                <textarea value={form.message} onChange={set('message')} rows={5} required
                  placeholder="현재 상황과 원하시는 내용을 자유롭게 적어주세요."
                  className={inputCls + ' resize-none'} />
              </div>
              <button type="submit" disabled={loading}
                className="btn-red w-full justify-center disabled:opacity-40 disabled:cursor-not-allowed">
                {loading ? (
                  <>
