...
import { Lock, ShieldCheck, Search, FolderOpen, ClipboardCheck, BarChart3 } from 'lucide-react'

const items = [
  { icon: Lock,           title: 'SSL/TLS 분석',  desc: '인증서 유효성, 암호화 강도, HSTS 설정 확인' },
  { icon: ShieldCheck,    title: '보안 헤더',      desc: 'CSP, X-Frame-Options, X-XSS-Protection 등 누락 여부' },
  { icon: Search,         title: '공개 취약점',    desc: 'CVE 데이터베이스 기반 사용 중인 라이브러리 취약점 조회' },
  { icon: FolderOpen,     title: '노출 정보',      desc: '오픈 디렉토리, 민감 파일 노출, 에러 정보 누출 확인' },
  { icon: ClipboardCheck, title: '입력 폼 점검',   desc: 'XSS 기초 점검, 비암호화 폼 전송 확인' },
  { icon: BarChart3,      title: '종합 리포트',    desc: '위험도 등급별 취약점 목록 + 조치 가이드 PDF' },
]
