...
const courses = [
  ...
  },
  {
    ...
    sub: '자체 호스팅·에이전트·보안 · IT 담당자·관리자',
    meta: '11편 30강 · 약 10시간',
  },
]

const faqs = [
  { q: '강좌(사내 출강)와 컨설팅·구축은 별도인가요?', a: '네. 강좌는 사내 출강 교육이고, 컨설팅·구축은 실제 도입·운영을 대행하는 별도 서비스입니다. 강좌만 듣고 직접 구축하거나, 강좌 없이 컨설팅만 받으셔도 됩니다. 함께 진행 시 할인 적용됩니다.' },
  ...
  { q: '우리 회사 데이터가 외부에 유출되지 않나요?', a: '구축한 AI는 독립 서버에서 운영되며, 외부 AI 서비스에 데이터를 전달하지 않도록 설계 가능합니다.' },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-28 pb-20 px-6 bg-background" style={{ background: 'radial-gradient(ellipse at top right, color-mix(in oklch, var(--primary) 20%, transparent) 0%, var(--background) 60%)' }}>
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <span className="inline-block bg-primary/20 border border-primary/40 text-primary text-xs font-bold px-3 py-1 rounded-full mb-6">기업 AI 도입 · 컨설팅 · 사내 출강</span>
            <h1 className="text-4xl md:text-5xl font-black text-foreground leading-tight mb-6 max-w-2xl">
              기업 AI 도입 및 컨설팅
            </h1>
            <p className="text-muted-foreground text-lg leading-relaxed mb-8 max-w-xl">
              반복 업무 자동화부터 사내 AI 인프라 구축까지 — 중소기업에 맞춰 설계·구축·교육을 원스톱으로 제공합니다.
            </p>
            <Link href="/contact"
              ...
              무료 AI 도입 분석 신청 <ArrowRight className="w-5 h-5" />
            </Link>
            <p className="mt-4 text-xs text-muted-foreground">상담 무료 · 100만원대부터 시작</p>
          </div>
      ...
      <section className="py-16 px-6 bg-secondary">
        <div className="max-w-5xl mx-auto">
          <div className="mb-8">
            <p className="text-xs font-mono font-bold tracking-[0.2em] uppercase text-primary mb-2">Enterprise Academy</p>
            <h2 className="text-2xl md:text-3xl font-black text-foreground tracking-tight mb-2">사내 출강 강좌</h2>
            <p className="text-sm text-muted-foreground">중소기업 도입·운영 노하우를 사내 교육으로 전달합니다. 커리큘럼은 공개, 강의 자료(PPT·스피커 노트)는 비밀번호 입력 후 다운로드.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {courses.map((c) => (
              <Link key={c.href} href={c.href}
                className="group rounded-2xl border-2 border-foreground/15 bg-card p-6 hover:border-primary/50 transition-colors">
                <p className="text-xs font-mono font-bold text-primary tracking-wider mb-3">{c.badge}</p>
                <h3 className="text-lg font-black text-foreground tracking-tight mb-1">{c.title}</h3>
                <p className="text-sm text-muted-foreground mb-3">{c.sub}</p>
                <p className="text-xs font-mono text-muted-foreground">{c.meta}</p>
                <p className="text-sm text-primary font-semibold mt-4 inline-flex items-center gap-1 group-hover:gap-2 transition-all">
      ...
      <section className="py-20 px-6 bg-background">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="border-l-4 border-primary bg-card px-6 py-4 rounded-r-xl">
            <p className="text-foreground font-semibold">2개 이상 해당된다면, 무료 도입 분석을 신청해보세요.</p>
          </div>
        </div>
      </section>
