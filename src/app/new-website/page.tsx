...
const stats = [
  ...
  { value: '0.3%', label: 'CTA 없는 사이트 문의 전환율' },
  { value: '2.1배', label: '전환율 설계 후 문의 증가 (자사 기준)' },
  { value: '63%', label: '국내 모바일 인터넷 이용 비율' },
]

const solutions = [
  { icon: Target,     title: '전환율 설계',    desc: '방문자가 "여기다"라고 느끼고 문의 버튼을 누르게 됩니다.' },
  ...
  { icon: Search,     title: 'SEO 기반 설계',  desc: '구글·네이버 상위 노출을 처음부터 고려해서 구조를 잡습니다.' },
]

const siteTypes = [
  { emoji: '🏢', name: '브랜드 사이트',  when: '회사·포트폴리오',   feature: '신뢰감 + SEO 중심' },
  { emoji: '🛒', name: '쇼핑몰·커머스',  when: '제품 판매',         feature: '결제·재고 시스템 포함' },
  { emoji: '🔧', name: '서비스 플랫폼',  when: '예약·멤버십·커뮤니티', feature: '기능 중심, 사용자 계정' },
  { emoji: '📣', name: '랜딩페이지',     when: '광고·프로모션',     feature: '단일 CTA, 빠른 론칭' },
]
...
const steps = [
  ...
  { num: '03', title: '개발',           duration: '2~4주', desc: '프론트엔드·백엔드 풀스택 + 관리자 페이지 개발. 진행 상황 매주 공유.' },
  { num: '04', title: '배포·인수인계', duration: '3일',   desc: '서버 세팅, 도메인 연결, SSL 설치까지 전부 대신해드립니다.' },
]

const plans = [
  { name: '랜딩페이지',     price: '100만원대~', items: ['단일 페이지', 'CTA 중심 설계'],            highlight: false },
  { name: '브랜드 사이트',  price: '200만원대~', items: ['다중 페이지', 'SEO 구조', '관리자 페이지'], highlight: true },
...
const faqs = [
  ...
  { q: '도메인과 서버(호스팅)도 대신 설정해주나요?', a: '네. 도메인 구매부터 서버 설정, SSL 인증서 설치까지 전부 대신해드립니다. 클라이언트분이 신경 쓸 부분이 없습니다.' },
